...
class Test():
   def __init__(self,para):
        ...
        self.button = Button(self.root,
                          text = 'Click Me',
                          command = self.combineFunc(self.funcA, self.funcB))
        self.but1 = Button(self,text = "exit",command =  exit)
        self.button.pack()
        self.root.mainloop()
...
while running == True:
    ...
    if(clicknumber <= 9 and winner == 1):
            ...
            label.pack()
            but1 = Button(text = "play again",command = callgame).pack()
            but2 = Button(text = "exit",command = exit).pack()
            checker = 91
            running = False
